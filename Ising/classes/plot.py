...
class figure:
    ...
    def makeImageList(self):
        for i, site in enumerate(self.delta):
            print site
            self.tmp = self.imageList[i]
            self.tmp[site[0]][site[1]] *= -1
            self.imageList.append(self.tmp)
    ...
    def animate(self):
        self.makeImageList() 
        self.ani = animation.FuncAnimation(self.fig, self.updatefig, init_func=self.init, frames=len(self.imageList), interval=0, blit=True, repeat_delay=0)
        return self.ani,
...
class show:
    ...
    def saveVideo(self):
        ...
        self.ani.save('output/animation-'+str(int(time.time()))+'-'+str(size)+'-'+str(steps)+'-'+str(temp)+'.mp4', writer=self.FFwriter, fps=50, extra_args=['-vcodec', 'libx264'])
